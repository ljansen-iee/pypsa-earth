...
snakemake solve_sector_networks --cluster-config config/cluster_config.yaml --cluster "sbatch -p {cluster.partition} -t {cluster.walltime} -c {cluster.cpus_per_task} --mem {cluster.mem_mb} -x {cluster.exclude}" --jobs 199 --latency-wait 60 --keep-going
